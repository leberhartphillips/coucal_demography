...
```{r, echo = FALSE}
status_dat_all %>% 
  filter(species %in% c("BC", "WBC")) %>% 
  group_by(species, sex, event) %>% 
  dplyr::summarise(n_inds = n_distinct(ind_ID)) %>% 
  mutate(stage = "juvenile")

status_dat_all %>% 
  filter(species %in% c("BC", "WBC")) %>% 
  group_by(species) %>% 
  dplyr::summarise(n_broods = n_distinct(nest_ID),
                   n_years = n_distinct(year),
                   min_year = min(year),
                   max_year = max(year))
                   

adult_sample_summary <- 
  detect_dat %>% 
  filter(species %in% c("BC", "WBC") & age_status == "A") %>% 
  group_by(species, sex) %>% 
  dplyr::summarise(n_obs = n_distinct(ring_ID)) %>% 
  mutate(stage = "adult")
```
...
WBC_boot_out <- 
  readRDS(file = "output/bootstraps/tidy/WBC_adult_survival_CJS_bootstrap_tidy.rds")
```
...
```{r}
# bind_rows(coucal_HSR %>% dplyr::select(trait, species, mean, CI_low, CI_high, n_nests, n_years),
# coucal_egg_survival %>% dplyr::select(trait, species, mean, CI_low, CI_high, n_nests, n_years))
```
...
saveRDS(object = parameter_distributions, 
        file = "output/bootstraps/tidy/parameter_distributions.rds")
```
...
```{r, echo = FALSE}
parameter_distributions <-
  readRDS(file = "output/bootstraps/tidy/parameter_distributions.rds")
```
...
# load output
BC_hazard_rate_boot_tidy <-
  readRDS("output/bootstraps/tidy/BC_haz_sur_ASR_boot_tidy_stoc_trans.rds")

# load output
WBC_hazard_rate_boot_tidy <-
  readRDS("output/bootstraps/tidy/WBC_haz_sur_ASR_boot_tidy_stoc.rds")
```
...
saveRDS(object = BC_treat_sensitivity_analysis, 
        file = "output/sensitivity_analysis/BC_treat_sensitivity_analysis_ASR_trans.rds")

saveRDS(object = BC_Mprime_sensitivity_analysis_male, 
        file = "output/sensitivity_analysis/BC_Mprime_sensitivity_analysis_male_ASR_trans.rds")

saveRDS(object = BC_Mprime_sensitivity_analysis_female, 
        file = "output/sensitivity_analysis/BC_Mprime_sensitivity_analysis_female_ASR_trans.rds")
...
```{r, echo=FALSE}
BC_treat_sensitivity_analysis <-
  readRDS(file = "output/sensitivity_analysis/BC_treat_sensitivity_analysis_ASR_trans.rds")

BC_Mprime_sensitivity_analysis_male <-
  readRDS(file = "output/sensitivity_analysis/BC_Mprime_sensitivity_analysis_male_ASR_trans.rds")

BC_Mprime_sensitivity_analysis_female <-
  readRDS(file = "output/sensitivity_analysis/BC_Mprime_sensitivity_analysis_female_ASR_trans.rds")
...
mapview(coucal_wp, zcol = "julian_date",
        layer.name = "Coucal offspring radio tagging",
        layers.control.pos = "topright")
```
...
#   group_by(species, ring_ID) %>% 
#   dplyr::summarise(min_time = min(date),
#                    max_time = max(date),
#                    n_dates = n_distinct(date)) %>% 
#   mutate(time_window = as.numeric(max_time - min_time)) %>% 
#   mutate(encounter_rate = time_window/n_dates) %>% 
#   ungroup() %>% 
#   group_by(species) %>% 
#   dplyr::summarise(mean_ER = mean(encounter_rate),
#             sd_ER = sd(encounter_rate),
#             mean_TW = mean(time_window),
#             sd_TW = sd(time_window))
```
...
time_window <-
  bind_rows(detect_status_BC, detect_status_WBC) %>%
  filter(ring_ID %!in% c("GN52396", "GN56850", "GN56873", "GN90552")) %>% 
  dplyr::select(year, ring_ID, std_week_last_obs, std_week_first_obs) %>% 
  mutate(time_window = std_week_last_obs - std_week_first_obs + 1) %>% 
  arrange(time_window)
...
bind_rows(Black_Coucal_adult_weekly_Burnham_ch %>% mutate(species = "BC"), 
          White_browed_Coucal_adult_weekly_Burnham_ch %>% mutate(species = "WBC")) %>% 
  filter(ring_ID %!in% c("GN52396", "GN56850", "GN56873", "GN90552", "GN52401")) %>% 
  mutate(event = ifelse(str_detect(ch, "11"), 1, 0)) %>% 
  mutate(encounter_freq = str_count(ch, "1")) %>% 
  mutate(encounter_freq = ifelse(event == 1, encounter_freq - 1, encounter_freq)) %>% 
  left_join(., time_window, by = c("ring_ID", "year")) %>% 
  mutate(encounter_rate = encounter_freq/time_window) %>% 
  group_by(species, sex) %>% 
  dplyr::summarise(mean_tw = mean(time_window),
                   max_tw = max(time_window),
                   min_tw = min(time_window),
                   sd_tw = sd(time_window),
                   mean_er = mean(encounter_rate),
                   max_er = max(encounter_rate),
                   min_er = min(encounter_rate),
                   sd_er = sd(encounter_rate),
                   n_inds = n_distinct(ring_ID),
                   n_years = n_distinct(year))
```
