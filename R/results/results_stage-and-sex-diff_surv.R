...
# run the sex_diff_survival() function on the cleaned bootstrap list
BC_sex_diff_survival_output <- 
  sex_diff_survival(BC_boot_out, niter = 250) %>% 
  mutate(species = "BC")

WBC_sex_diff_survival_output <- 
  sex_diff_survival(WBC_boot_out, niter = 250) %>% 
  mutate(species = "WBC")

# consolidate results
sex_diff_survival_output <- 
  bind_rows(BC_sex_diff_survival_output,
            WBC_sex_diff_survival_output) %>% 
...
surv_diff_plot <-
  ggplot(aes(y = difference, x = stage, fill = stage, color = stage), 
